...
class FastQreader():
    ...
    usrOptIn: user's input Option
    usrOptOut: user's output Option
    ...
    totLineCount: a count of the total lines in the input fastq file
    ...
    def giveChunks(self):
        ...
        print("Input Sequence processed with {} bad sequences or lines. FastQ records and lines have been skipped.".format(self.badSequenceCount), file= sys.stderr)
        print("Input has {}/{} empty lines".format(self.emptyLine, self.totLineCount), file = sys.stderr)
    ...
    def readChunk (self):
        ...
        for line in self.fname:
            lineCounter += 1 #Check the fastq record state (header, sequence,quality header, quality sequence)?
            self.totLineCount +=1 #Increment the counter for lines in the file.
            if lineCounter == 1 and line.startswith('@'):
                #Found a header line, check if ':' is in the first element and merge the first 2 elements if not with a ':' separator
                ...
                if yo.startswith('+') and len(yo) ==1 :
                    ...
                    self.badSequenceCount +=1
                    lineCounter = 0 #No quality header line found, reset record counter, increment bad sequence counter, take in the next line to stay in phase
                    #sys.stdin.readline()
                    continue;
                #lineCounter =4
            elif lineCounter == 4:
                ...
                else: #Increment bad sequence counter if the input file has messed up lengths for quality and nucleotide sequences and continue to next fastq record block
                    self.badSequenceCount += 1
                ...
                self.emptyLine +=1
                print("WARNING: Empty line found at line: {}".format(self.totLineCount), file= sys.stderr)
                continue;
...
class PhredQuality(object):
    ...
    def P33toP64(self, Qseq):
        ...
        if False: #functionality can be turned off for testing.
            pass
        translator = str.maketrans("".join([chr(x) for x in range(33,98)]),"".join([chr(y) for y in range(64,129)]))
    ...
    def P33toP33(self, Qseq):
        ...
        return (self.readHeader,self.inReadSeq, self.qualHeader,Qseq) # no change to the quality score return it
    ...
    def P64BtoP33(self, Qseq, readSeq):
        ...
        for base, score in zip(list(readSeq), list(Qseq)):
            ...
            if score == 'B': #evaluate for min score for this encoding
                ...
                corReadSeq.append(base) #otherwise take the existing called base
        translator = str.maketrans("".join([str(chr(x)) for x in range(66,129)]),\
                    str(chr(33))+"".join([str(chr(y)) for y in range(36,96)]))
...
def main(myCommandLine=None):
    ...
    for record in fileInObj.readChunk():
        if usrOptions['usrFastaOpt']: # if user specifies to output a fasta file, fasta file will be generated instead of converted fastq
            print(fileInObj.fastaOut(record[0],record[1]))
        else:
            print(str(record[0]))
            print(str(record[1]))
            print(str(record[2]))
