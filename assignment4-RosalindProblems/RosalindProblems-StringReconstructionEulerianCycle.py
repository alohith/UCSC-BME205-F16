...
class Graph:
    """docstring for Graph."""
    def __init__(self, streamIn, nodeList, edgeList):
        self.kmerSuffixSet = set()
        if nodeList == None:
            self.nodeList = dict()
        else:
    ...
    def makeNodesAndEdges(self,stream):
        ...
        for line in stream:
            try:
                kmerLen = int(line.rstrip())
            except ValueError:
                kmer = line.rstrip()
                # print(kmer)

                kmerPrefix = line.rstrip()[:-1]
                kmerSuffix = line.rstrip()[1:]
                # self.kmerSuffixSet.add(kmerSuffix)

                if kmerPrefix not in self.nodeList.keys():
                    self.nodeList[kmerPrefix] = Node(kmerPrefix)
                if kmerSuffix not in self.nodeList.keys():
                    self.nodeList[kmerSuffix] = Node(kmerSuffix)

                self.edges.append(Edge(self.nodeList[kmerPrefix], self.nodeList[kmerSuffix]))
            # print(kmerLen)

    def walkThePath(self):
        ''' TODO '''
        import random
        startPathNode = random.choice([node for node in self.nodeList.values() if len(node.waysOut())>len(node.waysIn)])
    ...
    def genomeInGraph(self):
        genomePath = self.walkThePath()
        genomePath.reverse()
        finalGenome = genomePath.pop().printNode()

        while len(genomePath)>0:
            finalGenome += genomePath.pop().printNode()[-1]
        return finalGenome

def main():
    aGraph = Graph(sys.stdin, None,None)
    print(aGraph.genomeInGraph())
