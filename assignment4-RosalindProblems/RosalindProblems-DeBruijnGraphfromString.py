    ...
    def deBruijnGraph(self):
        ...
        for kmer in self.kmerSet:
            ...
            if kmerPrefix not in adjacencyList.keys():
                adjacencyList[kmerPrefix] = [kmerSuffix]
...
def main():
    ...
    if int(line) is not ValueError:
        fileProvidedKlen = int(line.rstrip())
        # print(fileProvidedKlen)
        kmerOperator=KmerOperations(fileProvidedKlen,None,inputfile.readline().rstrip())

    kmerSet = kmerOperator.kmerSeqs()
